...
# TODO Durchschnittsertrag je Jahr
mn_jahr = fin %>%
  group_by(year) %>%
  summarise(mean(dt.ha))
...
mn_kreispa = fin %>%
  group_by(gen) %>%
  summarise(mn_dt.ha.a = mean(dt.ha)^2/mean(shape_area))

mapview(mn_kreispa)

# TODO Durchschnittsertrag je Feldfrucht je Kreis 
gennames = unique(fin$gen)

for (i in 1:38) {
  mn_cropkreis = fin %>%
    filter(gen == gennames[i]) %>%
    group_by(crop) %>%
    mutate(mndt = mean(dt.ha))
  paste0(mn_cropkreis, i)
}
